...
int bd_initfree(void *bd_left, void *bd_right) {
  ...
  for (int k = 0; k < MAXSIZE; k++) {  // skip max size
    ...
    if (bit_isset(bd_sizes[k].alloc, left/2))  {
    	...
    	lst_push(&bd_sizes[k].free, addr(k, left));  // put buddy on free list
    	free += BLK_SIZE(k);
    }
    ...
    if (bit_isset(bd_sizes[k].alloc, right/2))  {
    	...
    	lst_push(&bd_sizes[k].free, addr(k, right));  // put buddy on free list
    	free += BLK_SIZE(k);
    }
  }
  return free;
...
void bd_init(void *base, void *end) {
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    bd_sizes[k].alloc = p;
    // fill sz/2 bytes with 0 for alloc array
    memset(bd_sizes[k].alloc, 0, sz);
    // increase the pointer p by sz/2 bytes because *alloc is divided by 2 now
    p += sz;
  }
